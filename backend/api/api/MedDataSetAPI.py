...
class MedDataSetApi(APIView):
    ...
    def get(self, request):
        ...
        if name:
            med_data_set.filter(name__icontains=name)
        if date_create_from:
            med_data_set.filter(date_create__gte=date_create_from)
        if date_create_before:
            med_data_set.filter(date_create__lte=date_create_before)
        if source:
            med_data_set.filter(source=source)
        if import_status:
            med_data_set.filter(import_status=import_status)
